...
from GroovyChat.database import ROCKY
...
async def cb_handler(_, query: CallbackQuery):
    if query.data == "HELP":
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_ROCKY = ROCKY.find_one({"chat_id": query.message.chat.id})
            if not is_ROCKY:
                await query.edit_message_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ.**")
            if is_ROCKY:
                ROCKY.delete_one({"chat_id": query.message.chat.id})
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_ROCKY = ROCKY.find_one({"chat_id": query.message.chat.id})
            if not is_ROCKY:
                ROCKY.insert_one({"chat_id": query.message.chat.id})
                await query.edit_message_text(
